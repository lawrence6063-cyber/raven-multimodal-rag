...
本项目是一个功能完备的智能问答框架，定位为 **RAG 技术学习与工程实践的实战平台**：

#### 1️⃣ 实战驱动学习 (Learn by Doing)
项目架构本身就是 RAG 工程实践的"**活体答案**"。我们将经典工程考点直接融入代码设计，通过动手实践来巩固理论知识：
...
- **深度扩展**：保留完全模块化的内部结构，方便开发者替换组件、魔改算法，作为具备深度的工程实践项目。
- **扩展指引**：文档中会明确指出各模块的扩展方向与建议，帮助你在掌握基础后继续深入迭代。

#### 3️⃣ 配套技术资源 (Comprehensive Technical Materials)
本项目配套完整的技术资源，帮助你快速吃透项目：
...
#### 4️⃣ 知识点与原理梳理 (Principles & Knowledge Map)
针对每个模块，整理配套的原理解析：
- **📚 知识点清单**：这块涉及哪些理论知识需要提前学习（如 BM25 原理、FAISS 索引类型、Cross-Encoder vs Bi-Encoder）
- **❓ 常见问题**：结合项目代码讲解常见技术问题及参考思路

#### 5️⃣ 社区交流与持续迭代 (Community & Iteration)
- **经验分享**：使用本项目的反馈与最佳实践，都会汇总沉淀
- **问题讨论**：一起探讨项目中的技术问题与优化方向
- **持续更新**：从代码实现到原理梳理，形成完整的工程实践知识库
...
| A2 | 引入 pytest 并建立测试目录约定 | [x] | 2026-06-08 | pytest 配置、tests/ 目录结构、10 个冒烟测试 |
| A3 | 配置加载与校验（Settings） | [x] | 2026-06-08 | 配置加载、校验与 11 个单元测试 |

#### 阶段 B：Libs 可插拔层

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| B1 | LLM 抽象接口与工厂 | [x] | 2026-06-08 | BaseLLM + LLMFactory + 8个单元测试 |
| B2 | Embedding 抽象接口与工厂 | [x] | 2026-06-08 | BaseEmbedding + EmbeddingFactory + 5个单元测试 |
| B3 | Splitter 抽象接口与工厂 | [x] | 2026-06-08 | BaseSplitter + SplitterFactory + 4个单元测试 |
| B4 | VectorStore 抽象接口与工厂 | [x] | 2026-06-08 | BaseVectorStore + VectorStoreFactory + 6个单元测试 |
| B5 | Reranker 抽象接口与工厂（含 None 回退） | [x] | 2026-06-08 | BaseReranker + RerankerFactory + NoneReranker + 8个单元测试 |
| B6 | Evaluator 抽象接口与工厂 | [x] | 2026-06-08 | BaseEvaluator + EvaluatorFactory + CustomEvaluator + 10个单元测试 |
| B7.1 | OpenAI-Compatible LLM 实现 | [x] | 2026-06-09 | OpenAILLM + AzureLLM + DeepSeekLLM + 测试 |
| B7.2 | Ollama LLM 实现 | [x] | 2026-06-09 | OllamaLLM + 测试 |
| B7.3 | OpenAI & Azure Embedding 实现 | [x] | 2026-06-09 | OpenAIEmbedding + AzureEmbedding + 测试 |
| B7.4 | Ollama Embedding 实现 | [x] | 2026-06-09 | OllamaEmbedding + 测试 |
| B7.5 | Recursive Splitter 默认实现 | [x] | 2026-06-09 | RecursiveSplitter + 6个测试 |
| B7.6 | ChromaStore 默认实现 | [x] | 2026-06-09 | ChromaStore + get_by_ids |
| B7.7 | LLM Reranker 实现 | [x] | 2026-06-09 | LLMReranker + 5个测试 |
| B7.8 | Cross-Encoder Reranker 实现 | [x] | 2026-06-09 | CrossEncoderReranker + 4个测试 |
| B8 | Vision LLM 抽象接口与工厂集成 | [x] | 2026-06-09 | BaseVisionLLM 抽象接口 |
| B9 | Azure Vision LLM 实现 | [x] | 2026-06-09 | AzureVisionLLM + 3个测试 |

#### 阶段 C：Ingestion Pipeline MVP

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| C1 | 定义核心数据类型/契约（Document/Chunk/ChunkRecord） | [x] | 2026-06-09 | Document + Chunk + ChunkRecord + RetrievalResult |
| C2 | 文件完整性检查（SHA256） | [x] | 2026-06-09 | SQLiteIntegrityChecker + 10个测试 |
| C3 | Loader 抽象基类与 PDF Loader | [x] | 2026-06-09 | BaseLoader + PdfLoader(MarkItDown) |
| C4 | Splitter 集成（调用 Libs） | [x] | 2026-06-09 | DocumentChunker + 7个测试 |
| C5 | Transform 基类 + ChunkRefiner | [x] | 2026-06-09 | BaseTransform + ChunkRefiner(规则+LLM降级) |
| C6 | MetadataEnricher | [x] | 2026-06-09 | MetadataEnricher(title/summary/tags) |
| C7 | ImageCaptioner | [x] | 2026-06-09 | ImageCaptioner(Vision LLM + 降级) |
| C8 | DenseEncoder | [x] | 2026-06-09 | DenseEncoder(调用 EmbeddingFactory) |
| C9 | SparseEncoder | [x] | 2026-06-09 | SparseEncoder(BM25 term weights) |
| C10 | BatchProcessor | [x] | 2026-06-09 | BatchProcessor(合并 dense+sparse) |
| C11 | BM25Indexer（倒排索引+IDF计算） | [x] | 2026-06-09 | BM25Indexer(build/query/persist) |
| C12 | VectorUpserter（幂等upsert） | [x] | 2026-06-09 | VectorUpserter(stable ID) |
| C13 | ImageStorage（图片存储+SQLite索引） | [x] | 2026-06-09 | ImageStorage(save/get/list/delete) |
| C14 | Pipeline 编排（MVP 串起来） | [x] | 2026-06-09 | IngestionPipeline(6阶段+进度回调) |
| C15 | 脚本入口 ingest.py | [x] | 2026-06-09 | scripts/ingest.py(--path/--collection/--force) |

#### 阶段 D：Retrieval MVP

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| D1 | QueryProcessor（关键词提取 + filters） | [x] | 2026-06-10 | QueryProcessor + ProcessedQuery + 7个测试 |
| D2 | DenseRetriever（调用 VectorStore.query） | [x] | 2026-06-10 | DenseRetriever + 3个测试 |
| D3 | SparseRetriever（BM25 查询） | [x] | 2026-06-10 | SparseRetriever + get_by_ids + 3个测试 |
| D4 | RRF Fusion | [x] | 2026-06-10 | RRFFusion + 6个测试 |
| D5 | HybridSearch 编排 | [x] | 2026-06-10 | HybridSearch(Dense+Sparse+Fusion+降级) |
| D6 | Reranker（Core 层编排 + Fallback） | [x] | 2026-06-10 | QueryReranker + fallback + 3个测试 |
| D7 | 脚本入口 query.py（查询可用） | [x] | 2026-06-10 | scripts/query.py(--query/--top-k/--verbose/--no-rerank) |
...
| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| H1 | RagasEvaluator 实现 | [x] | 2026-06-11 | 惰性导入 ragas+可注入后端+优雅降级，注册 ragas provider，9 单测 |
| H2 | CompositeEvaluator 实现 | [x] | 2026-06-11 | 线程池并行+provider 前缀合并+单点失败降级，create_composite 工厂，8 单测 |
| H3 | EvalRunner + Golden Test Set | [x] | 2026-06-11 | EvalRunner/EvalReport 复用 HybridSearch，golden_test_set.json(5 条)，scripts/evaluate.py |
| H4 | 评估面板页面 | [x] | 2026-06-11 | evaluation_panel.py + eval_service.py，app.py 接入真实页面替换占位 |
| H5 | Recall 回归测试（E2E） | [x] | 2026-06-11 | test_recall.py 内存语料+确定性检索+hit@k/mrr 阈值，3 用例 |

#### 阶段 I：端到端验收与文档收口

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| I1 | E2E：MCP Client 侧调用模拟 | [x] | 2026-06-11 | test_mcp_client.py：子进程注入 fake 后端跑真实 stdio，走通 query_knowledge_hub 返回 citations，2 用例 |
| I2 | E2E：Dashboard 冒烟测试 | [x] | 2026-06-11 | test_dashboard_smoke.py：Streamlit AppTest 渲染 6 页面断言无异常，3 用例 |
| I3 | 完善 README（运行说明 + MCP + Dashboard） | [x] | 2026-06-11 | README 增「开发者指南」章节：安装/配置/ingest-query-evaluate/settings 字段/MCP 配置/Dashboard/测试 |
| I4 | 清理接口一致性（契约测试补齐） | [x] | 2026-06-11 | 补 vector_store(delete_by_metadata+stats)/reranker_factory/custom_evaluator 边界，契约 42 passed |
| I5 | 全链路 E2E 验收 | [x] | 2026-06-11 | scripts ingest/query/evaluate 离线冒烟通过 + 全量 291 passed |
...
- **M6（完成阶段 H+I）**：评估体系完整 + E2E 验收通过 + 文档完善，形成可复现的工程演示项目。
